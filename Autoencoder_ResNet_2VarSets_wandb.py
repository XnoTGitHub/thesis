...
#sweep_id = wandb.sweep(sweep_config, project="test 1 epochs")
print("init")
def run_model(config=None):
  ...
  train_dataloader, valid_dataloader, valid_dataloader_two = get_dataloaders(configs['Name'], configs['TRAIN_SET'], configs['VAL_SET'], configs['VAL_SET_TWO'], 8)# wandb.config.batch_size)
  ...
  if (TRAIN):
    ...
    #ptimizer = torch.optim.Adam(params=autoencoder.parameters() , lr=wandb.config.learning_rate, weight_decay=wandb.config.weight_decay)
...
#wandb.agent(sweep_id, run_model, count=5)
run_model()
print('finish')