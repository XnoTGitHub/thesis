    ...
    for view_dir in os.listdir(PATH_TO_RGB):
      ...
      for filename in os.listdir(PATH_TO_RGB + view_dir + '/'):
        if filename[-4:] in '.png.jpg':
          ...
          for i in range(32):
            if not os.path.isdir(PATH_TO_DEPTH + str(i) + '/'):
              os.mkdir(PATH_TO_DEPTH + str(i) + '/')
            Path_to_seg_image = PATH_TO_DEPTH  + str(i) + '/' + filename
            cropped_im = original_im.crop((0, 8+i*5, 640, 8+i*5 + 192))
            Suffix = Path_to_seg_image[-4:]
            Path_to_seg_image = Path_to_seg_image[:-4] + '_' + str(i) + Suffix
            print(Path_to_seg_image)
            width, height = cropped_im.size
            #print(width,height)
            #print(type(original_im))
            prep_im = np.transpose(cropped_im, (2, 0, 1 ))#change type to numpy
...
autoencoder.load_state_dict(torch.load('Autoencoder_ResNet18_DEPTH_OPT-FLOW.pt'))
...
if PROCESS_OPT:
  ...
  for town in os.listdir(PATH_TO_DATA):
    ...
    for i in range(32):
      ...
      PATH_TO_RGB = PATH_TO_DATA + town + '/pred_depth/' + str(i) + '/'
      PATH_TO_DEPTH = PATH_TO_DATA + town + '/pred_opt_same/'+ str(i) + '/'
